...
if(isset($_GET['id'])){
	$qry = $conn->query("SELECT * FROM user_productivityy where id = ".$_GET['id'])->fetch_array();
	foreach($qry as $k => $v){
		$$k = $v;
	}
}
...
<div class="container-fluid">
	<form action="" method="post" id="manage-progress" enctype="multipart/form-data">
    	...
    	$.ajax({
			...
			success:function(resp){
				if(resp == 1){
					...
					setTimeout(function(){
						window.location.replace("eviden.php");
					},1500)
				}
			}
    	})
