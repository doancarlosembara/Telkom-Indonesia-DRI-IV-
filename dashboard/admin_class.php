...
Class Action {
	...
	function login(){
		extract($_POST);
			$qry = $this->db->query("SELECT *, username as name FROM users where email = '".$email."' and password = '".($password)."'  ");
		if($qry->num_rows > 0){
	...
	function login2(){
		extract($_POST);
			$qry = $this->db->query("SELECT *, username as name FROM students where student_code = '".$student_code."' ");
		if($qry->num_rows > 0){
			foreach ($qry->fetch_array() as $key => $value) {
				if($key != 'password' && !is_numeric($key))
	...
	function save_progress(){
		...
		// echo "INSERT INTO user_productivityy set $data"; exit;
		if(empty($id)){
			$data .= ", user_id={$_SESSION['login_id']} ";
			
			$save = $this->db->query("INSERT INTO user_productivityy set $data");
		}else{
			$save = $this->db->query("UPDATE user_productivityy set $data where id = $id");
		}
		if($save){
			return 1;
		}
	...
	function delete_progress(){
		extract($_POST);
		$delete = $this->db->query("DELETE FROM user_productivityy where id = $id");
		if($delete){
			return 1;
		}
	}
